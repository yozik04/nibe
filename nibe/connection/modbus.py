...
from nibe.exceptions import CoilWriteException, CoilReadException, DecodeException
...
class Modbus(Connection):
    ...
    async def read_coil(self, coil: Coil, timeout: int = DEFAULT_TIMEOUT) -> Coil:
        ...
        try:
            result = await asyncio.wait_for(self._client.read_coils(slave_id=self._slave_id, starting_address=coil.address, quantity=1), timeout)
    ...
    async def write_coil(self, coil: Coil, timeout: int = DEFAULT_TIMEOUT) -> Coil:
        ...
        try:
            result = await asyncio.wait_for(self._client.write_coil(slave_id=self._slave_id, address=coil.address, value=coil.raw_value), timeout)
